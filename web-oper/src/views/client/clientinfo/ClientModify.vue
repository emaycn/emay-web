...
import { clientInfo, clientModify } from '@/api/client/client'
export default {
  name: 'ClientModify',
  data() {
  ...
  created() {
    clientInfo({ // 初始加载获取当前需要修改的客户信息
      id: this.$route.query.id
    }).then(response => {
      if (response.success === true) {
        var ruleForm = response.result
        this.ruleForm = ruleForm
        if (this.ruleForm.remark === null) {
          this.ruleForm.remark = ''
        } else {
          this.ruleForm.remark = this.transferText(this.ruleForm.remark)
        }
      }
    })
  },
    ...
    modify(formName) { // 修改客户信息
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          clientModify(this.ruleForm).then(response => {
            this.$message({
              message: '修改客户信息成功',
              type: 'success'
            })
