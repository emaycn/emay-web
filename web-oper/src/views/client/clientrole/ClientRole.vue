  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item v-if="AuthUtils.hasAuth('CLIENTROLE_ADD')" label>
          <router-link :to="{path: '/client/clientrole/add', query: { roleType:roleType}}">
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="400">
        <template slot-scope="{row}">
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTROLE_MODIFY') && row.id !=1"
            type="warning"
            size="mini"
            icon="el-icon-edit"
            title="编辑"
          ...
          />
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTROLE_DELETE') && row.id !=1"
            type="danger"
            size="mini"
            icon="el-icon-delete"
            title="删除"
...
<script>
import { fetchList, handleDeleteMethod } from '@/api/client/role' // 后台接口
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) { // 编辑
      this.$router.push({ path: '/client/clientrole/modify', query: { roleId: row.id, roleName: row.roleName, remark: row.remark, roleType: row.roleType }})
    },
