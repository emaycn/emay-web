  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item v-if="AuthUtils.hasAuth('CLIENTUSER_ADD')" label>
          <router-link to="/client/clientuser/add">
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="{row}">
          <!-- 0--停用,1--启用 -->
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTUSER_OPER') && row.userState === 0 && row.id!='1'"
            type="success"
            size="mini"
            title="启动"
            circle
          ...
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTUSER_OPER') && row.userState === 1 && row.id!='1'"
            type="info"
            size="mini"
            title="停止"
            circle
          ...
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTUSER_MODIFY') && row.id!='1'"
            type="warning"
            size="mini"
            icon="el-icon-edit"
            title="编辑"
          ...
          />
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTUSER_DELETE') && row.id!='1'"
            type="danger"
            size="mini"
            icon="el-icon-delete"
            title="删除"
          ...
          />
          <el-button
            v-if="AuthUtils.hasAuth('CLIENTUSER_OPER') && row.id!='1'"
            type="primary"
            size="mini"
            title="重置密码"
            circle
...
<script>
import { fetchList, handOnMethod, handOffMethod, handleDeleteMethod, handleResetMethod } from '@/api/client/user' // 后台接口
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) { // 编辑
      this.$router.push({ path: '/client/clientuser/modify', query: { id: row.id }})
    },
